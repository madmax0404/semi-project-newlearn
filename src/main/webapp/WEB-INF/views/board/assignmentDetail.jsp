...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/board/assignmentdetail.css" type="text/css" />
	...
	<div class="content">
		...
		<div class="board card shadow">
			<div class="board-detail-area">
				...
				<!-- 제출 게시글 정보 영역 -->
				<div class="board-detail-title" style="margin-top: 24px;">${board.boardTitle}</div>
				<div class="board-detail-meta">
					<span>작성자</span><span>${board.userName}</span> <span>작성일</span><span><fmt:formatDate value="${board.createDate}" pattern="yyyy-MM-dd HH:mm:ss" /></span> <span>제출일</span> <span> <fmt:formatDate value="${assignmentSubmission.submissionDate}" pattern="yyyy-MM-dd HH:mm:ss" />
					</span> <span>조회수</span><span>${board.viewCount}</span> <span>추천수</span><span>${likeCount}</span> <span>마감일</span> <span><fmt:formatDate value="${assignment.endDate}" pattern="yyyy-MM-dd HH:mm" /></span>
				</div>
